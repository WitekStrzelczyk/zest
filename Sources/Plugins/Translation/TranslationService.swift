...
final class TranslationService: @unchecked Sendable {
    ...
    private func translateWithAppleFramework(
        ...
        print(
            "🌐 translateWithAppleFramework: text='\(text)' target=\(targetLanguage) source=\(sourceLanguage ?? "nil")"
        )

        // Use Apple's Translation framework
        let source = if let sourceLang = sourceLanguage {
            Locale.Language(identifier: sourceLang)
        } else {
        ...
        do {
            ...
            let detectedSource: String? = translation.sourceLanguage.languageCode.flatMap(\.identifier)
